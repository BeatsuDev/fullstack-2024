<template>
    <form @submit.prevent="true">
        <select v-model="questionType">
            <option v-for="type in QuestionTypes" :key="type" :value="type">{{ getReadableQuestionType(type) }}</option>
        </select>
        <h4>Question</h4>
        <ValidatedInput
            id="question"
        ...
        <div v-else-if="questionType == QuestionTypes.MULTIPLE">
            <h4>Options</h4>
            <div v-for="(option, index) in editable.options" class="multiple-question-container"
                 :key="index">
                <ValidatedInput
            ...
            <ButtonComponent filled @click="addAnswer" v-if="editable?.options?.length < 4" style="margin-top: 20px">Add
                option
            </ButtonComponent>
        </div>
        ...
        <ButtonComponent filled style="margin-top: 20px" @click="submit">Submit</ButtonComponent>
    </form>
</template>
